...
export class SesionClienteComponent implements OnInit {
  ...
  ngOnInit(): void {
    if (localStorage.getItem('cliente')) {
      ...
      this.configService.loadConfig().subscribe({
        next: (res) => {
          console.log(res);
          const TIEMPO_ESPERA_INACTIVIDAD = res.find(
            (x) => x.ConfiguracionQuioscoId == 'TIEMPO_ESPERA_INACTIVIDAD'
          )?.Valor;
          const TIEMPO_ESPERA_CONTADOR = res.find(
            (x) => x.ConfiguracionQuioscoId == 'TIEMPO_ESPERA_CONTADOR'
          )?.Valor;
          ...
          if (
            ...
            TIEMPO_ESPERA_CONTADOR &&
            ID_QUIOSCO
          ) {
            this.quioscoId = parseInt(ID_QUIOSCO);
            this.idleUserService.setIdleTime(
              parseInt(TIEMPO_ESPERA_INACTIVIDAD)
            );
            this.idleUserService.setCountDownTime(
              parseInt(TIEMPO_ESPERA_CONTADOR)
            );
            this.idleUserService.reset();
            this.idleUserService.initListener();
          } else {
            console.error(
              "Configuración 'TIEMPO_ESPERA_INACTIVIDAD' o 'TIEMPO_ESPERA_CONTADOR' no encontrada."
            );
          }
          // this.idleUserService.reset()
          // this.idleUserService.initListener()
  ...
  onTipoImpresionChange(event: any) {
    ...
    // that.cliente.Estado = selectedValue;

    const clienteData = {
      ClienteId: this.clienteId, // Solo el ID si es necesario
    ...
    if (this.cliente.ClienteId) {
      ...
      this.quioscoService.GetPrintData(this.cliente.ClienteId).subscribe({
        next: (result) => {
          if (result.status) {
            this.idleUserService.reset();
            this.idleUserService.userInactive.subscribe((data: any) => {
  ...
  imprimir() {
    ...
    if (this.cantidad > 0 && this.cantidad <= this.tope) {
        ...
        next: () => {
          this.signalrService.receiveMessage().subscribe({
            next: (innerRes) => {
              ...
              this.mensajeProgressBar = `${innerRes.serie} - ${
                innerRes.impreso ? 'IMPRESO' : 'ERROR'
              }`;
      ...
      this.quioscoService
        .Print(this.quioscoId, this.sorteoId, this.clienteId, this.cantidad)
        .subscribe({
          next: (result) => {
            if (result.status) {
              this.modalRef.close();
